...
from TeleVompy.Utils import settings
...
async def cmd_start_deeplink(message: Message, command: CommandObject):
    ...
    if isinstance(deeplink_result, str):
        await message.answer(text=deeplink_result, message_effect_id=settings.MessageEffectSettings.PETARD)
